...
const Auth = () => {
  ...
  const checkTwoFactorStatus = async (userId: string) => {
    try {
      const {
        data: settings,
        error
      } = await supabase.from('security_settings').select('two_factor_enabled, two_factor_verified, totp_secret, two_factor_secret').eq('user_id', userId).maybeSingle();
  ...
  const handleSignIn = async (e: React.FormEvent) => {
    ...
    try {
      ...
      toast({
        title: 'Success',
        description: credentialResult.requires2FA ? 'Successfully signed in with two-factor authentication.' : 'Successfully signed in.'
      });
    } catch (error) {
  ...
  return <div className="min-h-screen flex items-center justify-center bg-gray-50">
      <Card className="w-full max-w-md">
        <CardHeader>
          <CardTitle>SSS Solutions UP</CardTitle>
          <CardDescription>
              ...
              <form onSubmit={handleSignIn} className="space-y-4">
                ...
                <div className="space-y-2 hidden">
                  ...
                  <div className="flex justify-start">
                    <InputOTP id="signin-otp" maxLength={6} value={otp} onChange={setOtp}>
                      <InputOTPGroup>
